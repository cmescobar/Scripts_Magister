...
def preprocessing_audio_OLD(signal_in, samplerate, model_name, lowpass_params,
    ...
    def _conditioning_signal(signal_in, samplerate):
        ...
        if samplerate < samplerate_des:
            print(f'Upsampling de la señal de fs = {samplerate} Hz a fs = 11025 Hz.') 
            new_rate = samplerate_des           
            audio_to = upsampling_signal(signal_in, samplerate, new_samplerate=new_rate)

        elif samplerate > samplerate_des:
            print(f'Downsampling de la señal de fs = {samplerate} Hz a fs = 11025 Hz.')
    ...
    # Aplicando la separación de fuentes
    resp_signal, heart_signal = \
                nmf_process(audio_to, samplerate_des, hs_pos=y_out2, 
                            interval_list=interval_list, 
                            nmf_parameters=nmf_parameters,
...
def preprocessing_audio(signal_in, samplerate, samplerate_nmf,*,model_name, 
    ...
    # Grabando archivos temporales
    sf.write('_temp_files/resp_signal_temp.wav', 
             resp_signal, samplerate=samplerate_nmf)
    sf.write('_temp_files/heart_signal_temp.wav', 
             heart_signal, samplerate=samplerate_nmf)
