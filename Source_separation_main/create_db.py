...
from math_functions import raised_cosine_modified, _db_relation
...
def get_heart_respiratory_sounds(dir_to_heart, db_option='SS', db_scale='abs', dB_value=0, version=1, 
    ...
    dB_value : float, optional
        Razón de dB que se desea entre el sonido respiratorio y el sonido cardiaco. Por
        defecto es 0 dB.
    ...
    if ausc_zone not in ['Anterior', 'Posterior', 'Both']:
        raise Exception('Opción para "ausc_zone" no válida.')
    
    if db_scale not in ['abs', 'dB']:
        raise Exception('Opción para "dB_scale" no válida.')
        ...
        if version == 1:
            if db_scale == 'abs':
                ...
                folder_save = f'Database_manufacturing/db_HR/Source Separation/v{version}/'\
                              f'Seed-{seed} - x - {a_heart}_Heart {a_resp}_Resp '\
                              f'{a_noise}_White noise'
            else:
                # Definición del directorio donde se guardará
                folder_save = f'Database_manufacturing/db_HR/Source Separation/v{version}/'\
                              f'Seed-{seed} - ({dB_value}dB) [Resp-Heart]'
        else:
            if db_scale == 'abs':
                # Definición del directorio donde se guardará
    ...
    for comb in tqdm(combinations, desc='Sounds', ncols=70):
        ...
        if db_scale == 'abs':
            ...
            hr_sound = _db_relation(resp_to_sum, heart_to_sum, db_rel_desired=dB_value)
        else:
...
if __name__ == '__main__':
    ...
    func_to = 'get_heart_respiratory_sounds'
    
    
    #################            RUTINA            #################
    
    
    if func_to == 'get_heart_respiratory_sounds':
        # Parámetros
        ausc_zone = 'Both'                  # 'Both', 'Anterior', 'Posterior'
        db_option = 'SS'                    # 'SS' de Source Separation
                                            # 'HS' de Heart Segmentation
        db_scale = 'abs'                    # 'abs', 'dB'
        db_value = None
        version = 2
        dB = 0
        ...
        dir_to_heart = f'Database_manufacturing/db_heart/Manual combinations v{version}/{ausc_zone}'
        
        get_heart_respiratory_sounds(dir_to_heart, db_option=db_option, db_scale=db_scale, 
                                     dB_value=dB, version=version, ausc_zone=ausc_zone, 
                                     a_heart=a_heart, a_resp=a_resp, a_noise=a_noise, seed=seed)
    
    
    elif func_to == 'generate_dictionary_W':
    ...
    elif func_to == 'get_heart_sounds_by_name':
    ...
    elif func_to == 'create_artificial_heart_sounds':
        filepath = 'Database_manufacturing/db_heart'
        s1_sel = 59
        s2_sel = 60
